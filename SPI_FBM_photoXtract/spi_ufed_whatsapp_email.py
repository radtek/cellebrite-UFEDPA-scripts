...
class SPIWhatsAppEmailsParser(object):
	...
	date_patterns = {"datetime_format_1" : "(?P<datetime>\d{2}/\d{2}/\d{2},\s{1}\d{2}:\d{2})"} #date_format_1 dd/MM/yyyy HH24:mm
	...
	class WhatsAppChatElement:
		def __init__(self, datetime, name, message, action):
			...
			#print datetime
			matchDateTime = re.match(r'(?P<day>\d{2})/(?P<month>\d{2})/(?P<year>\d{2}),\s{1}(?P<hour>\d{2}):(?P<minute>\d{2})',datetime)
			if not matchDateTime is None:
				dt = DateTime(int(matchDateTime.group('year'))+2000,int(matchDateTime.group('month')),int(matchDateTime.group('day')),int(matchDateTime.group('hour')),int(matchDateTime.group('minute')),0)
